...
import {
    ...
    BookOpen, Eye, EyeOff, ChevronRight, LayoutDashboard,
} from "lucide-react";
...
export default function AdminLearningHub() {
    ...
    const handleTogglePublish = async (item: LearningHubItem) => {
        await fetch("/api/learning-hub", {
            method: "PUT",
            headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
                    ...
                    <div className="flex items-center gap-5">
                        ...
                        <div className="hidden sm:flex items-center gap-1.5 text-foreground/20">
                            ...
                            <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-foreground/[0.03]">
                                ...
                                <span className="text-[11px] font-bold uppercase tracking-[0.15em] text-foreground/50">Learning Hub</span>
                            </div>
                        </div>
                    </div>
                ...
                <div className="flex items-center justify-between mb-10">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-foreground/35">{items.length} item{items.length !== 1 ? "s" : ""}</p>
                        </div>
                    </div>
                    <Link href="/admin/learning-hub/edit" className="group flex items-center gap-2.5 px-6 py-3 rounded-xl bg-accent text-white text-sm font-bold transition-all duration-300 hover:shadow-[0_0_32px_rgba(238,32,70,0.3)] hover:-translate-y-0.5">
                ...
                <div className="flex flex-col gap-4">
                    {items.length === 0 && (
                        <div className="text-center py-20 text-foreground/30 text-sm">
                            No items yet. Click &quot;Add New&quot; to create your first entry.
                        </div>
                    )}
                    {items.map((item) => (
                        <motion.div
                        ...
                        >
                            {item.coverImage && (
                                <div className="w-20 h-20 rounded-xl overflow-hidden flex-shrink-0 bg-foreground/5">
                            ...
                            <div className="flex-1 min-w-0">
                                ...
                                <h3 className="text-lg font-bold tracking-tight mb-1 truncate">{item.title}</h3>
                                <p className="text-sm text-foreground/45 line-clamp-2">{item.summary}</p>
                            </div>
                            <div className="flex items-center gap-2 flex-shrink-0">
                                ...
                                <Link href={`/admin/learning-hub/edit?id=${item.id}`} className="w-9 h-9 rounded-lg flex items-center justify-center bg-background text-foreground/40 hover:text-accent transition-all" style={{ boxShadow: "var(--neu-raised-sm)" }}>
